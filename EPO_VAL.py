...
class EPO_LP:
    def __init__(self, m, n, eps=1e-3):
        self.m = m          # task num
...
#         self.r = r          # reference vector
    ...
    def get_alpha(self, l, G,  G_val, loss_val, C=False, relax=False):
#         r = self.r if r is None else r
#         assert tf.shape(l)[1].eval() == tf.shape(G)[1].eval() == tf.shape(r)[1].eval() == self.m, "length != m"
#         rl, self.mu_rl, a = self.adjustments(l, r)       # rl : (1,m), mu_rl : float, a : (1,m),Adjustments
        C = tf.matmul(G, tf.transpose(G))    # tensor (m,m) : Gradient inner products, G^T G
        Ca = tf.matmul(G_val, tf.transpose(G))               # tensor (1,m) : d_bal^TG
        rl = tf.reshape(tf.reduce_mean(G * G_val, axis=1),[1,self.m])
        self.mu_rl = loss_val
        ...
        num_cons = self.m + 1
        # balance (Simplex)
        constraints_left_balance = tf.concat([tf.eye(self.m),tf.ones([1, self.m]), C,tf.ones([1, self.m])], axis=0,name='EPO_concat_l_38')                         # sum(self.alpha) == 1
        constraints_right_balance = tf.concat([tf.zeros_like(rhs),tf.ones([1, 1]), rhs,tf.ones([1, 1])], axis=1,name='EPO_concat_l_39')                           # C @ alpha >= rhs
        # descent (Relaxed)
        constraints_left_descent_relaxed = tf.concat([tf.eye(self.m),tf.ones([1, self.m]), C,tf.ones([1, self.m])], axis=0,name='EPO_concat_l_41')                 # sum(self.alpha) == 1
        constraints_right_descent_relaxed = tf.concat([tf.zeros_like(rhs),tf.ones([1, 1]), tf.zeros_like(rhs),tf.ones([1, 1])], axis=1,name='EPO_concat_l_42')    # C @ alpha >= 0
        # descent (Restricted)
        constraints_left_descent_restricted = tf.concat([tf.eye(self.m),tf.ones([1, self.m]), C,Ca], axis=0,name='EPO_concat_l_44')  # alpha @ Ca >= -neg(np.max(Ca))
        constraints_right_descent_restricted = tf.concat([tf.zeros_like(rhs),tf.ones([1, 1]), tf.zeros_like(rhs), tf.reshape(-neg(tf.reduce_max(Ca)),(1,1))], axis=1,name='EPO_concat_l_45')
        ...
        prob = SimplexSolver(self.m, 2*self.m + 2, constraints_left,tf.squeeze(constraints_right), tf.squeeze(obj))
        self.gamma, alpha = prob.solve()

        return alpha/to_den(tf.reduce_sum(alpha))

#     def mu(self, rl, normed=False):
# #         assert tf.equal(tf.shape(tf.where(rl < 0))[0].eval(), 0).eval(), "rl<0 \n rl={}".format(rl.eval())
#         m = tf.shape(rl)[1]
#         m = tf.cast(m, dtype=tf.float32)
#         l_hat = rl if normed else rl / (tf.reduce_sum(rl)+1e-6)
#         l_hat = tf.where(tf.equal(l_hat.eval(),0), tf.fill(tf.shape(l_hat),self.eps),l_hat)
